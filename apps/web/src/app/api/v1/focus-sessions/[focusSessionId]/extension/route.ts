...
import { extendFocusSchema } from "@/features/focus/transport/focus-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, input, params] = await Promise.all([
        focusActor(request),
        parseJson(request, extendFocusSchema),
        context.params,
      ]);
      const data = await getFocusService().extend(
        actor,
    ...
    "Focus Session extension failed",
  );
}
