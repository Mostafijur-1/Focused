...
import { pauseFocusSchema } from "@/features/focus/transport/focus-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, input, params] = await Promise.all([
        focusActor(request),
        parseJson(request, pauseFocusSchema),
        context.params,
      ]);
      const data = await getFocusService().pause(
        actor,
