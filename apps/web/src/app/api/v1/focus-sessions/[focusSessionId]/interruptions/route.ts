...
import { interruptionSchema } from "@/features/focus/transport/focus-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, input, params] = await Promise.all([
        focusActor(request),
        parseJson(request, interruptionSchema),
        context.params,
      ]);
      const data = await getFocusService().interrupt(
        actor,
    ...
    "Focus interruption logging failed",
  );
}
