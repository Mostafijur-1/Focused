...
import { terminalFocusSchema } from "@/features/focus/transport/focus-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, input, params] = await Promise.all([
        focusActor(request),
        parseJson(request, terminalFocusSchema),
        context.params,
      ]);
      const data = await getFocusService().complete(
        actor,
    ...
    "Focus Session completion failed",
  );
}
