...
import { advanceIntervalSchema } from "@/features/focus/transport/focus-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, input, params] = await Promise.all([
        focusActor(request),
        parseJson(request, advanceIntervalSchema),
        context.params,
      ]);
      const data = await getFocusService().advanceInterval(
        actor,
    ...
    "Pomodoro interval transition failed",
  );
}
