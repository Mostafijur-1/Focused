...
import { updateHabitSchema } from "@/features/habits/transport/habit-schemas";
import {
  habitActor,
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, { habitId }, input] = await Promise.all([
        habitActor(request),
        context.params,
        parseJson(request, updateHabitSchema),
      ]);
      const data = await getHabitService().update(
        actor,
