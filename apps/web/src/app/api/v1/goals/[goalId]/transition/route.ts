...
import { transitionGoalSchema } from "@/features/goals/transport/goal-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, params, input] = await Promise.all([
        goalActor(request),
        context.params,
        parseJson(request, transitionGoalSchema),
      ]);
      return apiSuccess(
        {
          data: await getGoalService().transition(
            actor,
    ...
    "Goal transition request failed",
  );
}
