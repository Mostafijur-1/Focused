...
import { linkGoalSchema } from "@/features/goals/transport/goal-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, params, input] = await Promise.all([
        goalActor(request),
        context.params,
        parseJson(request, linkGoalSchema),
      ]);
      return apiSuccess(
        {
          data: await getGoalService().link(
            actor,
