...
import { addMilestoneSchema } from "@/features/goals/transport/goal-schemas";
  ...
  return handleApiRoute(
    ...
    async (requestId) => {
      const [actor, params, input] = await Promise.all([
        goalActor(request),
        context.params,
        parseJson(request, addMilestoneSchema),
      ]);
      return apiSuccess(
        {
          data: await getGoalService().addMilestone(
            actor,
    ...
    "Milestone create request failed",
  );
}
