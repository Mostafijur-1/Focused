...
Authentication is a feature-owned Clean Architecture boundary inside the Next.js modular monolith. Route Handlers and React forms depend on application use cases; use cases depend on ports; Prisma, Neon, Resend, Upstash, JOSE, Argon2, and OAuth HTTP adapters implement those ports.
...
domain policy/types
       ^
Prisma | crypto | OAuth | Resend | Upstash adapters
```
...
| Control         | Decision                                                                                                                 |
| --------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Passwords       | Argon2id, 19 MiB memory, 2 iterations, parallelism 1; 12-128 characters; constant-work verification for unknown users    |
...
| One-time links  | Random token, digest-only storage, atomic single use; verification 30 minutes, password reset 15 minutes                 |
| Recovery        | Enumeration-safe accepted response; password reset increments permission version and revokes all sessions                |
...
|-- domain/          Lifetimes, password policy, identities, session types
|-- application/     Authentication/OAuth use cases, ports, RBAC policy
|-- infrastructure/  Prisma, crypto, provider, message, rate-limit adapters
|-- transport/       Zod request contracts, cookies, request security, responses
`-- ui/              Native copy, forms, in-memory session provider, session view
```
...
Copy `apps/web/.env.example` to `apps/web/.env.local`. Required for password Authentication:
...
If a refresh token is suspected compromised, revoke the session. If a user credential is compromised, use password reset, which revokes all sessions. For signing-key compromise, replace the pair, increment affected users' `permissionVersion` or revoke sessions, rotate OAuth transaction encryption material after short-lived transactions expire, and preserve audit evidence. Rotate Resend, Upstash, OAuth, Neon, and Vercel credentials at their providers, then update environment secrets and redeploy.
